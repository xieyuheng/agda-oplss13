  ...
  module Map where
    ...
    module ExplicitArgs where
      ...
      map-length : {A B : Set} (f : A -> B) (xs : List A) (n : Nat)
                 ...
                 -> Length (map f xs) n
      map-length f .[] .0 L[] = {!!}
      map-length f .(x :: xs) .(S n) (L:: n x xs h) = {!!}


      -- case l then n then len
      map-length2 : {A B : Set} (f : A -> B) (xs : List A) (n : Nat)
                 -> Length xs n
                 -> Length (map f xs) n
      map-length2 f .[] .0 L[] = {!!}
      map-length2 f .(x :: xs) .(S n) (L:: n x xs h) = {!!}
      
      -- case l then n then len
      map-length-inversion : {A B : Set} (f : A -> B) (xs : List A) (n : Nat)
                 -> Length xs n
                 -> Length (map f xs) n
      map-length-inversion f [] Z L[] = L[]
      map-length-inversion f [] (S n) ()
      map-length-inversion f (x :: xs) Z h = {!!}
      map-length-inversion f (x :: xs) (S n) h = {!!}
