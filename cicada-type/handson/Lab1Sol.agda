...
module handson.Lab1Sol where
  ...
  module Map where
    ...
    module ExplicitArgs where
      ...
      map-length : {A B : Set} (f : A -> B) (xs : List A) (n : Nat)
                 ...
                 -> Length (map f xs) n
      map-length f .[] .0 L[] = L[]
      map-length f .(x :: xs) .(S n) (L:: n x xs lxs) = L:: n (f x) (map f xs) (map-length f xs n lxs)
  ...
  module Nth where
    ...
    module Wrong where
      ...
      nth Z [] = {!!} -- nothing to return here, and no exceptions bc Agda is total
      nth Z (x :: l) = x
      nth (S n) l = nth n l
    ...
    {- TASK 6
       ...
       Some when the index is in bounds.  The type "M == N" is
       called propositional equality; it is an inductive family with
       one constructor
         Refl : M == M
    -}
    ...
    nth-spec FZ (L:: len) = _ , Refl
    nth-spec (FS i) (L:: len) = nth-spec i len
    ...
    nth' .(S n) .(x :: xs) (FZ {n}) (L:: {.n} {x} {xs} len) = x
    nth' .(S n) .(x :: xs) (FS {n} i) (L:: {.n} {x} {xs} len) = nth' n xs i len
  ...
  module Filter {A : Set} {P : A -> Set} (decideP : (x : A) -> Either (P x) (P x -> Void)) where
    ...
    filter-invariant [] = E[]
    filter-invariant (x :: xs) with decideP x
    ... | Inl yes = E:: yes (filter-invariant xs)
    ... | Inr no = filter-invariant xs
  ...
  module TreeInsertSorted (Keys : DecidableOrder) (Value : Set) where
    ...
    weaken low high (S-Empty l≤h) = S-Empty (≤-trans (≤-trans low l≤h) high)
    weaken low high (S-Node sl sr) = S-Node (weaken low ≤-refl sl) (weaken ≤-refl high sr)
    ...
    insert-sorted (S-Empty L≤H)  = S-Node (S-Empty min-≤-2) (S-Empty max-≥-2)
    insert-sorted {L}{H}{._}{k}{v} (S-Node{.L}{M}{.H} sl sr) with compare k M
    insert-sorted (S-Node sl sr) | Less k<M k!=M = S-Node (weaken ≤-refl (≤-trans (≤-refl' max-sym) (≤-refl' (max-≤ k<M))) (insert-sorted sl))
                                                          (weaken ≤-refl max-≥-1 sr)
    insert-sorted (S-Node sl sr) | Equal Refl = S-Node (weaken min-≤-1 ≤-refl sl) (weaken ≤-refl max-≥-1 sr)
    insert-sorted (S-Node sl sr) | Greater k>M k!=M = S-Node (weaken min-≤-1 ≤-refl sl) (weaken (≤-refl' (! (min-≤ k>M))) ≤-refl (insert-sorted sr))
